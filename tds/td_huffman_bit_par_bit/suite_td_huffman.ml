...
let table_occurrences (nomdefichier : string) : int array =
    ...
    with 
      |End_of_file -> close_in fichier; occs

(* Renvoie la liste des (F occs.(c), c) pour tous les caractères (i.e. entiers
 * de [0..255]) apparaissant au moins une fois. *)
let foret (occs : int array) : (arbre * int) list =
  let len = Array.length occs in
  let rec aux l =
    if l >= len then []
...
let construit_arbre (table_occs : int array) : arbre =
  let prioq = PrioQ.of_list (foret table_occs) in 
  while PrioQ.length prioq > 1 do
    let (c,k) = PrioQ.extract_min prioq in 
    let (c',k') = PrioQ.extract_min prioq in 
    PrioQ.insert prioq Noeud((c,c'),k + k')
  done;
  let t,n = PrioQ.extract_min prioq in 
  t

(* Renvoie un t : bool list array de taille 256 tel que t.(c) soit le code
 * associé à l'octet c. *)
let table_code (arbre : arbre) : bool list array =
  let t_bool = Array.make 256 [] in
  let rec remplir_tableau noeud chemin = 
    match noeud with
      |Feuille c -> t_bool.(c) <- List.rev chemin
      |Noeud(l,r) -> 
        remplir_tableau l (false :: chemin);
        remplir_tableau r (true :: chemin)
      in remplir_tableau arbre [];
  t_bool
...
let rec input_arbre (f : in_channel) : arbre =
  ...
  |0 -> let l = input_arbre f in 
        let r = input_arbre f in
        Noeud(l,r)
  |1 -> Feuille (input_byte f)
...
let output_bit (f : out_channel_bits) (b : bool) : unit =
  if b then
    f.o_accumulateur <- f.o_accumulateur + 1 lsl f.o_bits_accumules;
...
let close_out_bits (f : out_channel_bits) : unit =
  if f.o_bits_accumules = 0 then
    ...
    let a_ajouter = 8 - f.o_bits_accumules in 
    output_byte f.o_fichier f.o_accumulateur;
    output_byte f.o_fichier a_ajouter
  end;
...
let input_bit (f : in_channel_bits) : bool =
   if f.i_bits_accumules = 0 then begin
    ...
    if pos_in f.i_fichier = f.i_taille - 1 then begin
      let padding = input_byte f.i_fichier in
      f.o_bits_accumules <- f.o_bits_accumules - padding
    end
  end 
  let bit = (f.i_accumulateur mod 2) = 1 in
  f.i_accumulateur  <- f.i_accumulateur / 2 in
...
let rec decompresse_byte (f : in_channel_bits) (a : arbre) : int =
  match arbre with
  |F c -> c 
  |Noeud(l,r) -> decompresse_byte f (if input_bit f then r else l)

let compresse_fichier (nom_in : string) (nom_out : string) : unit =
  let table_occs = table_occurrences nom_in in
  let tree = construit_arbre occ in
  let f_out = open_out_bits nom_out in
  output_arbre f_out.o_fichier a;
  let a_plat = table_code a in
...
let decompresse_fichier (nom_in : string) (nom_out : string) : unit =
  ...
  let arbre = input_arbre f_in.i_fichier in
  let f_out = open_out_bin nom_out in
  begin
    try
      while true do 
        let c = decompresse_byte f_in arbre
      in output_byte nom_out f_out c 
    done;
    with 
      |End_of_file -> ()
    close_in_bits f_out;
